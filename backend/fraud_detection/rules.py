...
from .config import HIGH_RISK_CATEGORIES, ThresholdConfig, WeightConfig
...
SIGNAL_WEIGHT_KEYS: dict[str, str] = {
    ...
    "high_risk_category": "category_rarity",
    "amount_zscore_elevated": "amount_anomaly",
}
...
def evaluate_row(row: pd.Series, thresholds: ThresholdConfig) -> RuleResult:
    ...
    if zscore >= thresholds["amount_zscore_high"]:
        ...
        result.signals["amount_zscore_elevated"] = 0.45
        result.context["amount_zscore"] = round(zscore, 2)
    ...
    if ratio >= thresholds["amount_ratio_spike"]:
        ...
        result.signals["amount_ratio_moderate"] = 0.55
        result.context["amount_ratio"] = round(ratio, 1)
    ...
    if bool(row.get("velocity_burst", False)):
        ...
        if tx_5 >= thresholds["velocity_5min_count"]:
            result.signals["velocity_5min_high"] = 0.85
    ...
    elif bool(row.get("geo_mismatch", False)):
    ...
    if category in HIGH_RISK_CATEGORIES:
        strength = HIGH_RISK_CATEGORIES[category]
        result.signals["high_risk_category"] = strength
        result.context["merchant_category"] = category
